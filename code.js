...
async function generateSankeyDiagram(data) {
...
function calculateNodeLevels(data) {
    const levels = {};
    ...
    const queue = sourceNodes.map(id => ({ id, level: 0 }));
...
function calculateValueScale(data, nodeLevels) {
...
function calculateNodeLayouts(data, nodeLevels, valueScale, nodeValues, nodePercentages) {
    const layouts = [];
    // Group nodes by level
    const nodesByLevel = {};
    for (const [nodeId, level] of Object.entries(nodeLevels)) {
    ...
    for (const link of data.links) {
        if (!childrenOf[link.source])
            childrenOf[link.source] = [];
        childrenOf[link.source].push(link.target);
        if (!parentsOf[link.target])
            parentsOf[link.target] = [];
    ...
    const orderedNodesByLevel = {};
    const visited = new Set();
    function collectDescendantsInOrder(nodeId, targetLevel, result) {
    ...
    for (let level = 0; level <= maxLevel; level++) {
        ...
        if (!isLastLevel) {
            ...
            for (const nodeId of orderedNodes) {
                ...
                const dynamicPadding = Math.max(CONFIG.minNodePadding, CONFIG.labelHeight - height / 2);
            ...
            for (const leafId of orderedNodes) {
                ...
                if (primaryParent) {
                    if (!leafsByParent[primaryParent])
                        leafsByParent[primaryParent] = [];
...
function calculateLinkLayouts(data, nodeLayouts, valueScale) {
    const linkLayouts = [];
    // Track vertical position for each node's connections
    const nodeSourceOffsets = {};
    const nodeTargetOffsets = {};
    ...
    const sortedLinks = [...data.links].sort((a, b) => {
        ...
        if (!sourceNodeA || !sourceNodeB || !targetNodeA || !targetNodeB)
            return 0;
...
async function createChevronIcon() {
...
async function createNode(parent, node, data) {
...
async function createLink(parent, link, nodeLayouts) {
    const sourceNode = nodeLayouts.find(n => n.id === link.source);
    const targetNode = nodeLayouts.find(n => n.id === link.target);
    if (!sourceNode || !targetNode)
        return;
